    ...
    class CharacterMovement extends KeyAdapter {
        ...
        public void keyPressed(KeyEvent key) {
            ...
            switch (kc){
                case KeyEvent.VK_A: {
                    if(paused) return;
                    if (!level.collides(coords, Movement.WEST)) {
                        hero.move(Movement.WEST);
                    }
                    break;
                }
                case KeyEvent.VK_D: {
                    if(paused) return;
                    if (!level.collides(coords, Movement.EAST)) {
                        hero.move(Movement.EAST);
                    }
                    break;
                }
                case KeyEvent.VK_W: {
                    if(paused) return;
                    if (!level.collides(coords, Movement.NORTH)) {
                        hero.move(Movement.NORTH);
                    }
                    break;
                }
                case KeyEvent.VK_S: {
                    if(paused) return;
                    if (!level.collides(coords, Movement.SOUTH)) {
                        hero.move(Movement.SOUTH);
                    }
                    break;
                }
